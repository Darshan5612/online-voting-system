	...
	<div class="col-lg-12">
		<form action="" id="manage-opt">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="opt_name" pattern="[a-zA-Z]*" oninvalid="setCustomValidity('Please enter only characters')" name="opt_txt" value="<?php echo isset($opt_txt) ? $opt_txt :'' ?>">
			</div>
			<div class="form-group">
...
$('#manage-opt').submit(function(e)
{
		...
		var letters = /^[A-Za-z ]+$/;
		if(!(opt_name.match(letters)))
		{
				alert('Use only characters in name');
		}
		else
		{
		start_load()
